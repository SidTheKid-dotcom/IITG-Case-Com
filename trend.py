...
plt.figure(figsize=(8, 6))
sns.boxplot(x=not_churned_data['Tenure'], color="lightgreen")
...
for i, col in enumerate(numerical_columns, 1):
    plt.subplot(2, 2, i)  # For female data
    sns.boxplot(x='Churn', y=col, data=female_data, palette='Set2')
    plt.title(f"Female: {col} vs Churn")
    
    plt.subplot(2, 2, i + 2)  # For male data
    sns.boxplot(x='Churn', y=col, data=male_data, palette='Set1')
...
for i, col in enumerate(categorical_columns, 1):
    plt.subplot(2, 2, i)
    churn_rate = female_data.groupby(col)['Churn'].mean() * 100  # Churn rate in percentage
...
for i, col in enumerate(categorical_columns, 1):
    plt.subplot(2, 2, i)
    churn_rate = male_data.groupby(col)['Churn'].mean() * 100  # Churn rate in percentage
...
# 4. Feature Importance with Statistical Significance (Bar Plot)
plt.figure(figsize=(10, 6))
drivers = feature_importances.copy()
drivers['Statistical_Significance'] = [
    ttest_ind(female_data[feature], female_data[female_data['Churn'] == 0][feature], nan_policy='omit')[1]
    if feature in numerical_columns else None
    for feature in drivers['Feature']
]
sns.barplot(x='Importance', y='Feature', data=drivers, palette='viridis')
plt.title("Feature Importance with Statistical Significance")
...
plt.figure(figsize=(10, 6))
sns.scatterplot(x='Age', y='BillingDelay', hue='Sex', data=data, palette='Set2')
